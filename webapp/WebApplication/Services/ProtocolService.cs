...
namespace K9.WebApplication.Services
{
    public class ProtocolService : CacheableServiceBase<Protocol>, IProtocolService
    {
        ...
        public Protocol AutoGenerateProtocolFromGeneticProfile(int clientId, bool refresh = false)
        {
            var hq = _healthQuestionnaireService.GetHealthQuestionnaireForClient(clientId);
            if (hq != null)
            {
                return AutoGenerateProtocolFromGeneticProfile(hq, refresh);
            }
            return null;
        }

        public Protocol AutoGenerateProtocolFromGeneticProfile(HealthQuestionnaire hq, bool refresh = false)
        {
            ...
            else
            {
                ...
                protocol.Client = _clientsRepository.Find(hq.ClientId);
            }
            ...
            if (refresh)
            {
                _protocolActivitiesRepository.DeleteBatch(protocol.Activities);
                _protocolDietaryRecommendationRepository.DeleteBatch(protocol.DietaryRecommendations);
                _protocolFoodItemsRepository.DeleteBatch(protocol.RecommendedFoods);
                _protocolProductsRepository.DeleteBatch(protocol.Products);
                _protocolProductPackRepository.DeleteBatch(protocol.ProductPacks);
            }
